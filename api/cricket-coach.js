...
// structured coaching report. Groq's free tier currently has no
// vision-capable model available (confirmed live against /v1/models), so
// this runs on the numeric pose-estimation metrics alone rather than key
// frame images. Tries each candidate text model in order and moves on if
// one is unavailable, since Groq's free-tier lineup keeps changing.
...
analysis if provided. Identify patterns: consistency, dismissal patterns, scoring trends by year/format,
and — when a recent technical session is provided — explicitly connect statistical patterns to technical
causes (e.g. "a high rate of caught-behind dismissals lines up with the head-falling-over flaw measured in
your last session"). Compare pragmatically to what separates club/age-group level from serious
professional-level batting (typical benchmarks: a first-class average is generally 35+, a serious
professional/international-track average is generally 45+, with strong conversion of starts (30+) into
big scores (100+); use these only as general orientation, never claim to know this specific player's
national-team prospects). Be honest about where the numbers are weak, and specific about what closing the
gap requires.

Always respond with STRICT JSON only, no markdown fences, matching this schema:
{
  "career_score": <integer 0-100, overall statistical strength for their level>,
  "summary": "<3-5 sentence assessment of the career record>",
  "strengths": [ "<statistical strength>", ... ],
